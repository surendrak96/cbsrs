...
<!--------------Content--------------->
<section id="content">
	<div class="wrap-content cbsrs" align='center'>
		<div class="row block01">
			<div class="col-full">
				<div class="wrap-col" align='left'>

				<form action='studentmarkslist.php' method='POST' enctype="multipart/form-data">				
				<table border="0">
				<tr>
				...
				<tr>
					<td></td>
					<td align="left"><input class="btn" type="Submit" value="View Marks" name="Show_Marks" style='min-width:180px;height:34px;color:white;border-style: solid;border-width:1px;'/><br>
					</td>
				</tr>
				<tr>
				<td colspan=2>
...
<?php
			...
			if(isset($_POST['sem']) && isset($_POST['Show_Marks']))
				{
						
						$sem 	= mysql_real_escape_string($_POST['sem']);
						
						$success = mysql_query("SELECT * from `student_marks` where htno='$userid' AND `branch`='$branch' AND `sem`='$sem'") or die(mysql_error());
						...
						if($htnos_count==$subject_count)
							{
								$get_rec = mysql_fetch_array($success);
									...
									echo "
										...
										$get_marks = mysql_query("SELECT * from `student_marks` where htno='$userid' AND `branch`='$branch' AND `sem`='$sem'") or die(mysql_error());
